...
drink_list = json.load(drink_file)  # load and parse the JSON
drink_file.close()
...
# These 'users' are always present
special_names = ['everyone', 'the house', 'here', 'me']
...
def build_menu_list():  # this has to go here, because python won't let me forward declare it,
    # but I also can't call it before it's declared. Python, man.
    drink_keys = drink_list.keys()
    menu_num_pages = math.ceil(len(drink_keys)/config["menu_page_size"])
    ...
    for x in range(menu_num_pages):
        ...
        for i in range(config["menu_page_size"]):
            drink_name = drink_list[sorted_drink_keys[drink_counter]].get(
                "name")
            drink_desc = drink_list[sorted_drink_keys[drink_counter]].get(
                "menudesc")
            curr_embed.add_field(
                name=drink_name, value=drink_desc, inline=False)
...
async def serve(ctx):
    ...
    modified_content = content.lower().replace(
        '!bb - serve', '', 1).split('-')  # strip out the formatting
    ...
    # Someone who suggests a drink can always serve it,
    elif ctx.author.id in [int(x) for x in curr_drink["users"]]:
       ...
       # print(allowed_to_serve)
        ...
        # This will not handle multiple matching roles well
        for role in role_list:
            if role.name in config["server_roles"]:
                serve_pronoun = config["server_roles"][role.name]
                break
            if role.name in allowed_roles:
                allowed_to_serve = True
                break

    if not allowed_to_serve:
        await ctx.send("Sorry, " + ctx.author.display_name + ", but you aren't allowed to serve that.")
        return

    # Is the recipient tagged as a minor?
    if curr_drink["alcoholic"] and not self_flag and not all_flag:
    ...
    embed = build_embed(ctx, recipient, curr_drink,
                        serve_pronoun, self_flag, all_flag)
...
async def menu(ctx, page_no=1, category='all'):
    ...
    await menu_page.add_reaction("\N{BLACK LEFT-POINTING TRIANGLE}")
    # "\N{BLACK RIGHT-POINTING TRIANGLE}"
    await menu_page.add_reaction("\N{BLACK RIGHT-POINTING TRIANGLE}")
...
async def on_reaction_add(reaction, user):
    ...
    if user.id == bot.user.id:
        ...
        if emote == "BLACK LEFT-POINTING TRIANGLE":
            ...
            # "\N{BLACK LEFT-POINTING TRIANGLE}"
            await reaction.message.add_reaction("\N{BLACK LEFT-POINTING TRIANGLE}")
            # "\N{BLACK RIGHT-POINTING TRIANGLE}"
            await reaction.message.add_reaction("\N{BLACK RIGHT-POINTING TRIANGLE}")
...
def build_embed(ctx, recipient, curr_drink, serve_pronoun, self_flag, all_flag):
    ...
    if self_flag == True:
        title_string = ctx.author.display_name + ' pours ' + \
            serve_pronoun + article + curr_drink["name"]
    elif all_flag == True:
        title_string = ctx.author.display_name + \
            ' pours the House a round of ' + curr_drink["name"]
    else:
        title_string = ctx.author.display_name + ' pours ' + \
            recipient.display_name + article + curr_drink["name"]

    picture_url = curr_drink["pic"]

    embed = discord.Embed(title=title_string, color=0xffffff,
                          description=curr_drink["desc"])
    embed.set_image(url=picture_url)
    embed.add_field(name="So, what's in the glass?",
                    value=curr_drink["menudesc"], inline=False)
